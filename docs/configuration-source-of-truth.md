...
- `hugo.toml` owns `defaultContentLanguage` and `contentDir`
...
- params blocks
- pagination
- content root and public route behavior
...
2. If the setting is not there, inspect `config/_default/hugo.toml`.
...
- `hugo.toml` owns the English-only content root and public route contract.
